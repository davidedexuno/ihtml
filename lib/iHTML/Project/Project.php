...
class Project
{
	...
	function render($out)
	{
		...
		if(file_exists($out_dir) && ( !is_dir($out_dir) || !is_writable($out_dir) )) throw new \Exception('Error creating output folder.');
		if(!file_exists($out_dir) && !mkdir($out, 0777, true))                       throw new \Exception('Error creating output folder.');
		
		// SOLVE IMPLICIT FILES
		foreach($this->templates as &$template)
			if(substr($template->output, -1) == '/') $template->output .= $this->implicit;
		unset($template);
