...
interface CcsInterface
{
	...
	function setContent(string $content, string $root): self;

	function getHierarchyList(): array;

	function getHierarchyTree(): array;

	function applyTo(Document $document): self;
...
class Ccs implements CcsInterface
{
	...
	function setContent(string $content, string $root): CcsInterface
	{
	...
	function getHierarchyList(): array
	{
		...
		if($this->content === false) throw new \Exception('Ccs Content not set.');
	...
	function applyTo(Document $document): CcsInterface
	{
	...
	private function loadRules()
	{
		foreach(glob(dirname(__FILE__).'/Rules/*.class.php') as $ruleFile)
		{
			...
			if(!class_exists($className)) throw new \Exception('Class '.$className.' doesn\'t exists.');
	...
	private function loadRule($rule)
	{
        ...
        '/^((?<type>'.$types.')-)?/i' => function ($match) use (&$type) { if(isset($match['type'])) $type = $match['type']; return ''; },
        '/(-(?<prop>'.$props.'))?$/i' => function ($match) use (&$prop) { if(isset($match['prop'])) $prop = $match['prop']; return ''; },
    ], $rule);

		if($type == false) $type = in_array($name, self::CSS_RULES) ? 'style' : 'class';
		if($prop == false) $prop = ['style' => 'literal', 'class' => 'visibility', 'attr' => 'content'][$type];
